...
from django.views import View  # pyright: ignore[reportMissingImports]
...
class CreateSurveyFormView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        try:
            question_payload = self._extract_questions(request)
        ...
        try:
            with transaction.atomic():
                ...
                self._create_questions(survey, question_payload)
        except Exception as exc:
            context['error'] = f'Unable to save survey: {exc}'
            return render(request, 'my_app/survey_create.html', context)
    ...
    def _extract_questions(self, request):
        """Gather question definitions from POST data."""
        raw_indexes = request.POST.getlist('question_indexes')
        if not raw_indexes:
            return []  # Allow creating surveys without questions
        ...
        for idx in raw_indexes:
            ...
            q_type = request.POST.get(f'question_type_{idx}', 'text')
            required = request.POST.get(f'question_required_{idx}') == 'on'
            
            if not text:
                raise ValueError('Each question needs a prompt.')
            if q_type not in valid_question_types:
                raise ValueError('Invalid question type supplied.')
            
            choices = []
            if q_type in {'mcq', 'likert'}:
                raw_choices = request.POST.getlist(f'choice_{idx}[]')
                choices = [choice.strip() for choice in raw_choices if choice and choice.strip()]
                if len(choices) < 2:
                    raise ValueError('Multiple choice and Likert questions need at least two choices.')
    ...
    def _create_questions(self, survey, questions):
        """Persist questions and related choices."""
        for payload in questions:
            ...
            if payload['choices']:
                Choice.objects.bulk_create(
                    [Choice(question=question, text=choice_text) for choice_text in payload['choices']]
                )
...
class SurveyResponsesAnalyticsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, survey_id, **kwargs):
        ...
        responses_qs = (
            ...
            .select_related('student', 'student__profile', 'student__profile__section')
        ...
        if search_term:
            responses_qs = responses_qs.filter(
                models.Q(student__username__icontains=search_term)
                | models.Q(student__first_name__icontains=search_term)
                | models.Q(student__last_name__icontains=search_term)
            )

        if date_from:
            start_dt = datetime.combine(date_from, time.min, tzinfo=timezone.get_current_timezone())
            responses_qs = responses_qs.filter(submitted_at__gte=start_dt)
        if date_to:
            end_dt = datetime.combine(date_to, time.max, tzinfo=timezone.get_current_timezone())
            responses_qs = responses_qs.filter(submitted_at__lte=end_dt)

        paginator = Paginator(responses_qs, 5)
        ...
        for response in responses:
            ...
            for answer in response.answers.all():
                if answer.selected_choice:
                    student_answers['answers'][answer.question.id] = answer.selected_choice.text
                else:
                    student_answers['answers'][answer.question.id] = answer.text_answer or 'No answer'
            
            response_data.append(student_answers)
        
        query_params = self.request.GET.copy()
        if 'page' in query_params:
            query_params.pop('page')
        context['survey'] = survey
        context['total_responses'] = responses_qs.count()
        context['responses'] = responses
        context['page_obj'] = page_obj
        context['paginator'] = paginator
        context['questions'] = questions
        context['response_data'] = response_data
        context['filter_search'] = search_term
...
def _build_response_analytics(user, survey_id=None):
    stopwords = {
        'the', 'and', 'for', 'this', 'that', 'with', 'have', 'your', 'from',
        'you', 'are', 'was', 'were', 'not', 'but', 'all', 'any', 'can',
        'just', 'about', 'their', 'they', 'them', 'our', 'out', 'when',
        'what', 'who', 'why', 'how', 'had', 'has', 'too', 'very', 'more'
    }
    answers_qs = Answer.objects.filter(question__survey__created_by=user)

    survey_meta = {'id': None, 'title': 'All Surveys'}
    survey_obj = None
    if survey_id:
        try:
            survey_obj = Survey.objects.get(id=survey_id, created_by=user)
            answers_qs = answers_qs.filter(question__survey=survey_obj)
            survey_meta = {'id': survey_obj.id, 'title': survey_obj.title}
        except Survey.DoesNotExist:
            pass

    answers = answers_qs.select_related('selected_choice', 'question__survey').prefetch_related('question__choices')
    mcq_counter: Counter[str] = Counter()
    likert_counter: Counter[str] = Counter()
    text_counter: Counter[str] = Counter()

    # Collect all Likert choice labels from questions to maintain order
    likert_ordered_labels = []
    if survey_obj:
        likert_questions = survey_obj.questions.filter(question_type='likert').prefetch_related('choices')
    else:
        likert_questions = Question.objects.filter(
            question_type='likert',
            survey__created_by=user
        ).prefetch_related('choices')
    
    # Get unique Likert labels in order (use first question's order as reference)
    seen_labels = set()
    for question in likert_questions:
        for choice in question.choices.all().order_by('id'):
            label = choice.text.strip()
            if label and label not in seen_labels:
                likert_ordered_labels.append(label)
                seen_labels.add(label)

    for answer in answers:
        q_type = answer.question.question_type
        if q_type == 'mcq' and answer.selected_choice:
            label = answer.selected_choice.text.strip() or 'No answer'
            mcq_counter[label] += 1
        elif q_type == 'likert' and answer.selected_choice:
            label = answer.selected_choice.text.strip() or 'No selection'
            likert_counter[label] += 1
        elif q_type == 'text' and answer.text_answer:
            tokens = re.findall(r'\b\w+\b', answer.text_answer.lower())
            for token in tokens:
                if len(token) < 3 or token in stopwords or token.isdigit():
                    continue
                text_counter[token] += 1

    # Use ordered labels if available, otherwise use frequency-based ordering
    likert_chart_data = _format_chart_data(
        likert_counter,
        ordered_labels=likert_ordered_labels if likert_ordered_labels else None,
    )

    data = {
        'mcq_chart_data': _format_chart_data(mcq_counter),
        'likert_chart_data': likert_chart_data,
        'word_cloud_data': _format_word_cloud(text_counter),
        'survey_meta': survey_meta,
    }
    return data


def _format_chart_data(counter: Counter[str], limit: int = 8, ordered_labels=None):
...
def _format_word_cloud(counter: Counter[str], limit: int = 30):
    if not counter:
        return []
    entries = counter.most_common(limit)
    max_count = entries[0][1]
    min_scale, max_scale = 0.6, 1.2
    span = max(max_count, 1)
    return [
        {
            'word': word,
            'count': count,
            'weight': round(min_scale + (count / span) * (max_scale - min_scale), 2),
        }
...
class TeacherDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not profile or profile.role != 'teacher':
            raise PermissionDenied("Access denied. Only teachers can access this page.")
        ...
        context['analytics_surveys'] = surveys.values('id', 'title')
        ...
        active_section = self.request.GET.get('section', 'overview')
        context['active_section'] = active_section if active_section in {'overview', 'manage'} else 'overview'
        default_survey_id = self.request.GET.get('analytics_survey') or None
        context.update(_build_response_analytics(self.request.user, default_survey_id))
        
        return context


class TeacherDashboardAnalyticsView(LoginRequiredMixin, View):
    """AJAX endpoint returning up-to-date analytics for teacher dashboard charts."""
...
class StudentDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not profile or profile.role != 'student':
            raise PermissionDenied("Access denied. Only students can access this page.")
        ...
        context['student_display_name'] = f"Student {first_name}" if first_name else f"Student {self.request.user.username}"
